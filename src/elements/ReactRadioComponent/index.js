...
export default class ReactRadioGroup extends Component {
  ...
  setCheckedRadio = () => {
    const value = this.props.value != null ? this.props.value : this.state.defaultValue;
  ...
  handleChange = (event) => {
    ...
    const fn = this.props.onChange || emptyFn;
  ...
  prepareChildren = (props, state) => {
    const checkedValue = props.value != null ? props.value : state.defaultValue;

    return (props.items || []).map((item, index, arr) => {
      const inputStyle = assign({}, props.inputStyle);
      const labelStyle = assign({}, props.labelStyle);
      const disabledStyle = assign({}, props.disabledStyle);
      let disabled = false;
      let checked;
      let value;
      let children;
      ...
      const radioProps = {
        ...
        label: children,
        item,
        children: [<input {...inputProps} />, children],
      };

      const renderFn = props.renderRadio;
      let result;
...
ReactRadioGroup.propTypes = {
  ...
  items(props, name) {
    if (!props.children && !props.items) {
      return new Error(
        "Your component has no children. In this case, you should specify an items array.",
      );
    }
  },
};
